...
class DatabaseAccess(object):
    ...
    def update_record(self, coll, spec, record, replace=False):
        if replace:
            ...
            self.mongo.run('update', coll, in_bson=spec,
                           value_bson={'$set': record})

    def upsert_draft_record(self, coll, spec, record):
        if coll != 'metadata':
                return self.mongo.run('upsert', coll, key_bson=spec,
                                      value_bson=record, ups=True)
        else:
    ...
    def get_rawfile_processed_status(self, fn):
        return self.mongo.run('find_one', 'metadata',
                              ...
                              in_fields=['files.{0}.extension'.format(fn),
                                         'general_info'])
