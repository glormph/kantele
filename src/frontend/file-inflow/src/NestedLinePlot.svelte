...
import { plotLegend } from '../../dashboard/src/PlotLegend.js'
import { drawLabels } from '../../dashboard/src/AxisLabels.js'
import { drawGridLines } from '../../dashboard/src/GridLines.js'
...
export async function plot() {
  ...
  arr_series.forEach(serie => {
    plotgroup.append('path')
      ...
      .attr("d", line()
        .x(function(d) { return xScaleValues(d[xkey]) })
        .y(function(d) { return yScaleValues(d[serie]) })
      );
    if (data.map(d => d[serie]).length == 1) {
      plotgroup.append('circle')
        .data(data)
        .attr("fill", d => color(serie))
        .attr("stroke", d => color(serie))
        .attr("cx", d => xScaleValues(d[xkey]) + jitter(xScaleValues.step()))
        .attr("cy", d => yScaleValues(d[serie]))
        .attr("r", 5)
    }
  });
