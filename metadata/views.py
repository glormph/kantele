...
def select_files(request, dataset_id):
    return dataset_view_action(request, dataset_id, 'file_input.html',
                               'metadata')


@login_required
def write_metadata(request, dataset_id):
    return dataset_view_action(request, dataset_id, 'base_meta.html', 'store')
...
def store_dataset(request, dataset_id):
    if request.method == 'GET':
        ...
        if not mds:
            ...
            return redirect('/kantele')  # Can we add a message here?


def post_dataset(mds, request, dataset_id, template, nextstep=None):
...
def return_get(request, template, ds_id, mds, message=None):
    return render(request, 'metadata/{0}'.format(template),
                  {'mds': mds, 'ds_id': ds_id, 'message': message})
...
def dataset_view_action(request, dataset_id, template, nextstep=None):
    ...
    if not mds:
        ...
        post_dataset(mds, request, dataset_id, template, nextstep)
...
def check_dataset_owner(request, dataset_id):
    if request.method not in ['POST', 'GET']:
        ...
        return deepcopy_metadataset(empty_mds)
            ...
            [x.mongoid for x in
             DraftDataset.objects.filter(user=request.user.pk)]:
        return deepcopy_metadataset(empty_mds)
    else:
        return False
